...
cisalhamento <- read.table(
  ...
  header = T)
names <- cisalhamento$especie
value <- cisalhamento$MOR2
...
data %>%
  ...
  ggtitle("Cisalhamento") +
  ylab("MOR (MPa)") + xlab("Espécies")+ ylim(200,750)+coord_flip ()
...
data %>%
  ...
  ggtitle("Cisalhamento") +
  ylab("MOR (MPa)") + xlab("Espécies")+ ylim(200,750)
