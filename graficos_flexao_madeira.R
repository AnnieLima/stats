...
flexao <- read.table(
  "C:/Users/Annie de Lima/Downloads/Estatística/stat_debs/flex_mad.txt",
  header = T)
names <- flexao$especie
value <- flexao$MOE
...
data %>%
  ...
  ggtitle("Flexão") +
  ylab("MOE (MPa)") +   xlab("Espécies")+coord_flip ()
...
data %>%
  ...
  ggtitle("Flexão") +
  ylab("MOR (MPa)") +   xlab("Espécies")+coord_flip ()
