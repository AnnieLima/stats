...
compressao <- read.table(
  "C:/Users/Annie de Lima/Downloads/Estatística/stat_debs/tensao.txt",
  header = T)
names <- compressao$especie
value <- compressao$tensao2
...
data %>%
  ...
  ggtitle("Compressão") +
  ylab("Resistência (kgf/cm²)") +  
  xlab("Espécies") + coord_flip ()
...
data %>%
  ...
  ggtitle("Compressão") +
  ylab("Resistência (kgf/cm²)") + xlab("Espécies")+coord_flip ()
...
data %>%
  ...
  ggtitle("Compressão") +
  ylab("Resistência (kgf/cm²)") +  xlab("Espécies")
